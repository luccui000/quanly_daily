<div class="flex-grow h-full" style=" overflow: auto">  
    <div class=""> 
        <div class="flex justify-between">
            ...
            <div class=""> 
                <x-button.primary type="button" wire:click="themMoiMatHang({{ count($danhsachNhapHang) + 1}})">
            ...
            <x-table>
                ...
                <x-slot name="body">    
                    @foreach ($danhsachNhapHang as $index => $nhaphang)
                        <x-table.row wire:key="danhsachNhapHang[{{ $index }}][id]">    
                            <x-table.cell>{{  $loop->iteration }}</x-table.cell>
                            <x-table.cell>
                                <select 
                                    name="danhsachNhapHang[{{ $index }}][id]"
                                    wire:model="danhsachNhapHang.{{ $index }}.id" 
                                ...
                                <div class="relative flex space-x-1">  
                                    ...
                                    <div class="">
                                        <input type="number"
                                            ...
                                            wire:model="danhsachNhapHang.{{ $index }}.SoLuong">
                                    </div>
                            ...
                            <x-table.cell> 
                                <div class="relative">
                                    <input  type="number" min="0" max="100" step="5" value="0" 
                                        ...
                                        wire:model="danhsachNhapHang.{{ $index }}.GiamGia" style="width: 40px">
                            ...
                            <input type="hidden" name="danhsachNhapHang[{{ $index }}][GiamGia]" value="{{ $nhaphang['SoLuong'] * (float)$nhaphang['DonGia'] * ($nhaphang['GiamGia'] / 100) }}">
                            <input type="hidden" name="danhsachNhapHang[{{ $index }}][DonGia]" value="{{ $nhaphang['DonGia'] }}">
                            <input type="hidden" name="danhsachNhapHang[{{ $index }}][ThanhTien]" value="{{ $nhaphang['SoLuong'] * (float)$nhaphang['DonGia'] - ($nhaphang['SoLuong'] * (float)$nhaphang['DonGia'] * ($nhaphang['GiamGia'] / 100)) }}">
                        </x-table.row> 
                    @endforeach  
                </x-slot>
            </x-table> 
